...
function ReqManager(options) {
    ...
    this.map = [];
    this.resolve_names = [];
    this.onChange = options.listener || function() {};
}
...
ReqManager.prototype.add = function(filename) {
    ...
    this.onChange(null, {action:'remove', filename:filename, resolve_names:this.resolve_names[filename]});
...
ReqManager.prototype.remove = function(filename) {
    ...
    this.onChange(null, {action:'remove', filename:filename, resolve_names:resolve_names});
}
